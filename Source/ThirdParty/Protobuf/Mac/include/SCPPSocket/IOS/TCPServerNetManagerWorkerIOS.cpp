...
{
    void TCPServerNetManagerWorkerIOS::DoWork()
    {
        int server_sockfd;
        int result;
        ...
        while(IsWorking)
        {
            ...
            if(result < 1)
            {
                printf("IOS server select return errno=%d\n", errno);
                perror("IOS server select return error");
    ...
    void TCPServerNetManagerWorkerIOS::HandleAccept()
    {
        int client_sockfd;
    ...
    void TCPServerNetManagerWorkerIOS::HandleRead()
    {
        int nread;
...
//                    delete[] msg;
    ...
    TCPServerNetManagerWorkerIOS::TCPServerNetManagerWorkerIOS()
    {
        std::printf("construct TCPServerNetManagerWorkerIOS\n");
    }

    TCPServerNetManagerWorkerIOS::~TCPServerNetManagerWorkerIOS()
    {
        std::printf("destruct TCPServerNetManagerWorkerIOS\n");
    }

    void TCPServerNetManagerWorkerIOS::SendMessage(int FileDescriptor, const char* Msg, int Len)
    {
    ...
    void TCPServerNetManagerWorkerIOS::Broadcast(const char *Msg, int Len)
    {
        std::list<Connection*>::iterator p1;
        for(p1=ConnectionsToClient.begin();p1!=ConnectionsToClient.end();p1++)
        {
    ...
    void TCPServerNetManagerWorkerIOS::StopWork()
    {
        delete[] LenBuf;
        LenBuf = nullptr;
        delete[] ReadBuf;
        ...
        std::printf("TCPServerNetManagerWorkerIOS StopWork\n");
    }
}
